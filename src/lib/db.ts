...
/**
 * Read the Postgres URL at call time (not module load).
 * Key names are looked up dynamically so the bundler cannot replace them with
 * an empty string at build. Vercel/Neon often inject POSTGRES_URL, not DATABASE_URL.
 */
export function readPostgresUrl(): string | undefined {
  const env = typeof process === "undefined" ? undefined : process.env;
  if (!env) return undefined;
  for (const key of POSTGRES_URL_KEYS) {
    const value = env[key];
    if (typeof value === "string" && value.trim() && /^(postgres(ql)?:\/\/)/i.test(value.trim())) {
      return value.trim();
    }
  }
  return undefined;
...
export function getDbSource(): DbSource {
  return readPostgresUrl() ? "neon" : "pglite";
}

/**
 * Active backend: real **Neon/Postgres** when a postgres URL is set, otherwise
 * embedded **PGLite**. Prefer getDbSource() on the request path.
 */
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    if (!url) throw new Error("Postgres URL missing at Neon pool create");
