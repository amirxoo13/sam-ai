...
| `amirxo13/iran-legal-corpus` (مخزن moshir) | قوانین ساخت‌یافته اختبار | **۱۳۰٬۱۷۱ ماده — متن کامل** هر ماده با تبصره و بند، بدون کوتاه‌سازی. |
| `amirxoo13/power` (شاخص ۹۹۹۶ پرونده) | رویهٔ دادرسی (`case_law`) | **۹٬۹۹۶ پرونده — رکورد کامل** عین فایل منبع، بدون حذف فیلد. |
| پایلوت OCR مشیر | دو متن تمیزشده | متن کامل هر دو فایل `02_cleaned_*`. |
| didban8 + اسناد moshir-documents | قوانین / کنوانسیون / نظریه / اصطلاح‌نامه | اسکریپت ingest آماده است؛ فایل‌های خام روی دیتاست **خصوصی** HuggingFace‌اند و بدون `HF_TOKEN` وارد نشدند. |
