#!/usr/bin/env node
/**
 * Embed cleaned qavanin.ir TreeText chunks and merge into legal-seed.json.
 * Keeps every existing db07 + jsonl + ekhtebar + case_law chunk.
 */
...
const BATCH = 8;
const CACHE_PATH = join(ROOT, "data/raw/qavanin/embed-cache.json");
...
function stripExtra(chunk) {
  const { _kind: _k, ...rest } = chunk;
  return rest;
}

async function main() {
  const selectedPath = join(ROOT, "src/data/qavanin-selected.json");
  ...
  const seed = {
    ...
    statuteAccess:
      "Core codes from db07; jsonl + ekhtebar PDFs; plus cleaned TreeText from qavanin.ir (pages 1–102, Arvan empties dropped).",
    counts,
