#!/usr/bin/env node
/**
 * Embed the filtered persian-legal-rag-jsonl subset and merge into legal-seed.json.
 * Keeps every existing db07 + case_law chunk (no re-embed of the 4989).
 */
...
const CACHE_PATH = join(
  ROOT,
  "data/raw/persian-legal-rag-jsonl/embed-cache.json",
);
...
async function main() {
  const selectedPath = join(ROOT, "src/data/jsonl-selected.json");
  ...
  const seed = {
    ...
    statuteAccess:
      "Core codes from user db07 LawItem.xlsx plus filtered unique statutes/opinions from github.com/amirxoo13/persian-legal-rag-jsonl (not the raw 82k dump).",
    counts,
